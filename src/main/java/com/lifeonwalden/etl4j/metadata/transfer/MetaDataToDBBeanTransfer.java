...
public class MetaDataToDBBeanTransfer implements MetaDataTransfer {
  ...
  private TypeSpec build(ResultSetMetaData rsmd) throws SQLException {
    ...
    Builder beanBuilder =
        TypeSpec.classBuilder(_className).addModifiers(Modifier.PUBLIC).addSuperinterface(Serializable.class).addField(
    ...
    for (int i = 1; i <= colCount; i++) {
      ...
      beanBuilder.addField(FieldSpec.builder(fieldClass, fieldName, Modifier.PRIVATE).build())
          .addMethod(MethodSpec.methodBuilder(StringUtils.join("set", methodSubfix)).addModifiers(Modifier.PUBLIC)
              ...
              .addStatement("this.$L = $L", fieldName, fieldName).addStatement("return this").build())
          .addMethod(MethodSpec.methodBuilder(StringUtils.join("get", methodSubfix)).addModifiers(Modifier.PUBLIC)
              .returns(fieldClass).addStatement("return this.$L", fieldName).build());
    }
