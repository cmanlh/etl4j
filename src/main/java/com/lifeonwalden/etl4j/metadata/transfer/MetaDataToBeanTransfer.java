...
public class MetaDataToBeanTransfer implements MetaDataTransfer {
     ...
     * @param className
     */
    public MetaDataToBeanTransfer(String packageName, String className) {
                ...
                if ("TABLE".equalsIgnoreCase(tableType) || "VIEW".equalsIgnoreCase(tableType)) {
                    JavaFile.builder(
                            ...
                            build(connection
                                    .createStatement()
                                    .executeQuery(
                                            FetchMetaStatement.fetch(tableResultSet.getString(3), prdName, driverName))
                                    .getMetaData())).build().writeTo(output);
                }
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
                ...
                if ("TABLE".equalsIgnoreCase(tableType) || "VIEW".equalsIgnoreCase(tableType)) {
                    JavaFile.builder(
                            ...
                            build(connection
                                    ...
                                    .executeQuery(
                                            FetchMetaStatement.fetch(tableResultSet.getString(3), prdName, driverName))
                                    .getMetaData())).build().writeTo(outputLocation);
                }
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
    ...
    private TypeSpec build(ResultSetMetaData rsmd) throws SQLException {
        ClassName _className =
                ClassName.get(packageName.trim(),
                        StringUtils.isBlank(className) ? rsmd.getTableName(1) : className.trim());
        Builder beanBuilder =
                TypeSpec.classBuilder(_className)
                        ...
                        .addField(
                                FieldSpec
                                        .builder(long.class, "serialVersionUID", Modifier.PRIVATE, Modifier.STATIC,
                                                Modifier.FINAL).initializer("$L", System.currentTimeMillis()).build());
        ...
        for (int i = 1; i <= colCount; i++) {
            ...
            beanBuilder
                    ...
                    .addMethod(
                            MethodSpec.methodBuilder(StringUtils.join("set", methodSubfix))
                                    .addModifiers(Modifier.PUBLIC).returns(_className)
                                    .addParameter(fieldClass, fieldName)
                                    .addStatement("this.$L = $L", fieldName, fieldName).addStatement("return this")
                                    .build())
                    .addMethod(
                            MethodSpec.methodBuilder(StringUtils.join("get", methodSubfix))
                                    .addModifiers(Modifier.PUBLIC).returns(fieldClass)
