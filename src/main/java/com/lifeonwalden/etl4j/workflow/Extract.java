...
public class Extract {
  ...
  public void query(List<Object> paramList) throws SQLException {
    if (rs != null) {
      ...
      statement =
          connection.prepareStatement(SqlParser.isValidSql(sql) ? sql : StringUtils.join("select * from ",
    ...
    if (null == columnLabelMapping) {
      ...
      try {
        for (int i = 1; i <= colCount; i++) {
          Column column =
              new Column().setClazz(Class.forName(rsmd.getColumnClassName(i))).setIndex(i)
  ...
  public Map<String, Object> fetchNextWithLabel() throws SQLException {
    ...
    for (Entry<String, Column> entry : columnLabelMapping.entrySet()) {
      ...
      result.put(column.getName(), rs.getObject(column.getIndex()));
    }
  ...
  public List<Object> fetchNextWithIndex() throws SQLException {
    ...
    for (Entry<String, Column> entry : columnLabelMapping.entrySet()) {
      Column column = entry.getValue();
      result[column.getIndex() - 1] = rs.getObject(column.getIndex());
    }
